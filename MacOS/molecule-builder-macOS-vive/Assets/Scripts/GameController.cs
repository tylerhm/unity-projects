...
public class GameController : MonoBehaviour
{
	...
	InputHandler inputHandlerLeft;
	InputHandler inputHandlerRight;

	public GameObject leftController;
	public GameObject rightController;

	private bool firstBond;

	void Start ()
	{
		inputHandlerLeft = leftController.GetComponent<InputHandler> ();
		inputHandlerRight = rightController.GetComponent<InputHandler> ();
	...
	void Update ()
	{
		if (inputHandlerRight.touchpad.x < 0) {
			firstBond = true;
		} else {
			firstBond = false;
		}
	...
	public void ActivateMenu ()		//activates menu when left finger trigger is pressed
	{
		...
		if (inputHandlerLeft.trigger) {
			menuActivity = true;
		} else if (!inputHandlerLeft.trigger) {
			menuActivity = false;
		}
	...
	public void SelectAtom ()		//if atom selected, remove right hand, and instantiate a atom to right hand.
	{
		...
		if (Physics.Raycast (ray, out hit, Mathf.Infinity) && inputHandlerRight.trigger) {														
			if (hit.collider.tag == "Button") {
				if (hit.transform.name == "CarbonButton") {
					...
					CreateAtom (sulfur);
				} else if (hit.transform.name == "FlorineButton") {
					CreateAtom (florine);
	...
	public void DropAtom ()		//drops the atom, when the right hand trigger is pressed
	{
		...
		if (atomSelected == true && !inputHandlerRight.trigger) {
	...
	public void DeleteObject ()		//deletes object if looking at it, and trigger is pressed
	{
		...
		if (Physics.Raycast (ray, out hit, Mathf.Infinity) && inputHandlerRight.grip) {
	...
	public void TrashMenuActivation ()		//activates the trash scene menu
	{
		...
		if (Physics.Raycast (ray, out hit, Mathf.Infinity) && inputHandlerRight.trigger && atomSelected == false) {
	...
	public void PointOne ()		//picks first point for the bond, and then LOCKS everything else until second point is picked
	{
		...
		if (Physics.Raycast (ray, out hit, Mathf.Infinity) && inputHandlerRight.touchpress && (inputHandlerRight.touchpad.x < -0.3f)) {
	...
	{
		if (inputHandlerRight.touchpress && ((inputHandlerRight.touchpad.x > -0.3) && (inputHandlerRight.touchpad.x < 0.3))) {
			scaling = true;
		} else {
			scaling = false;
		}
